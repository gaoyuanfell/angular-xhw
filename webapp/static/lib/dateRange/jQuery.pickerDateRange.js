/**  https://github.com/zou12e/pickerDateRange
...
function pickerDateRange(inputId, options) {
    ...
    var wrapper = {
        gri: [
            ...
            '</div>',
            '<div>',
            '<input disabled type="button" name="' + this.submitBtn + '" id="' + this.submitBtn + '" value="确定" />',
        ...
        ta: [
            ...
            '<div class="ta_calendar_cont cf" id="' + this.dateListId + '"' + (this.mOpts.calendars != 1 ? 'style=""' : '') + '>',
            ...
            (this.mOpts.shortbtn ? '<div style="padding-bottom:5px;"> <div class="short-btn" id="aYesterday">昨天</div> <div class="short-btn" id="aToday">今天</div> <div class="short-btn" id="aRecent7Days">最近一周</div> <div class="short-btn" id="aRecent14Days">最近半个月</div><div class="short-btn" id="aRecent30Days">最近一个月</div><div class="short-btn" id="aRecent90Days">最近三个月</div></div>' : '') + '<div><div class="frm_msg">',
            ...
            '</div>',
            '<div class="frm_btn">',
            '<input disabled class="ta_btn ta_btn_primary" type="button" name="' + this.submitBtn + '" id="' + this.submitBtn + '" value="确定" />',
    ...
    $('#' + this.compareCheckboxId).bind('click', function () {
        ...
        if ($('#' + __method.compareCheckboxId).attr('checked')) {
            ...
            if ('' == scDate || '' == ecDate) {
                ...
                if (ecDate.getTime() < __method.str2date(__method.mOpts.minValidDate).getTime()) {
                    scDate = sDate;
                    ecDate = eDate;
                }
                if (ecDate.getTime() >= __method.str2date(__method.mOpts.minValidDate).getTime() && scDate.getTime() < __method.str2date(__method.mOpts.minValidDate).getTime()) {
                    scDate.setTime(__method.str2date(__method.mOpts.minValidDate).getTime())
...
pickerDateRange.prototype.init = function (isCompare) {
    ...
    for (var property in __method.periodObj) {
        if ($('#' + property).length > 0) {
            var id = property;
            if (!this.mOpts.singleDateRange) {
                id = this.calendarId + " #" + property;
            }
            $('#' + id).unbind('click');
...
pickerDateRange.prototype.getSpecialPeriod = function (period) {
    ...
    var timeStamp = ((date.getTime() - ( period * 24 * 60 * 60 * 1000)) < (__method.str2date(__method.mOpts.minValidDate).getTime())) ? (__method.str2date(__method.mOpts.minValidDate).getTime()) : (date.getTime() - ( period * 24 * 60 * 60 * 1000));
...
pickerDateRange.prototype.addCSS = function (isCompare, specialClass) {
    ...
    if (this.mOpts.theme == 'ta') {
        ...
        if (isNaN(sDate.getTime()) && isNaN(eDate.getTime())) {
            $('#' + this.calendarId + '_' + this.date2ymd(new Date()).join('-')).removeClass().addClass(this.mOpts.clickCss);
        }
    }
};
...
pickerDateRange.prototype.selectDate = function (ymd) {
    ...
    if (this.startDateId == this.dateInput) {
        ...
        //控制用户输入全部时间
        if(this.mOpts.calendarsChoose == 1){
        ...
        //控制用户输入全部时间
        if(this.mOpts.calendarsChoose == 2){
            $("#"+this.submitBtn).removeAttr("disabled")
        }
...
pickerDateRange.prototype.show = function (isCompare, __method) {
    ...
    if (0 < clientWidth && $("#" + this.calendarId).width() + pos.left > clientWidth) {
        left = pos.left + $('#' + this.inputId).outerWidth() - $("#" + this.calendarId).width() + ((/msie/i.test(navigator.userAgent) && !(/opera/i.test(navigator.userAgent))) ? 5 : 0);
        __method.mOpts.theme == 'ta' && (left);
    }
    $("#" + this.calendarId).css('left', left + 'px');
    //$("#" + this.calendarId).css('top', pos.top + (__method.mOpts.theme=='ta'?35:22) + 'px');
    $("#" + this.calendarId).css('top', pos.top + offsetHeight + 2 + 'px');
    //第一次显示的时候，一定要初始化输入框
...
pickerDateRange.prototype.close = function (btnSubmit) {
    ...
    if (btnSubmit) {
        ...
        if (this.mOpts.shortOpr === true) {
            ...
            $('#' + this.inputId).val($('#' + this.startDateId).val().replace(/\-/ig,'') + ('' == $('#' + this.endDateId).val() ? '' : this.mOpts.defaultText + $('#' + this.endDateId).val().replace(/\-/ig,'')));
        }
        ...
        var _val = this.mOpts.shortOpr == true ? $('#' + this.startDateId).val() : ($('#' + this.startDateId).val().replace(/\-/ig,'') + ('' == $('#' + this.endDateId).val() ? '' : this.mOpts.defaultText + $('#' + this.endDateId).val().replace(/\-/ig,'')));
        // 把开始、结束时间显示到输入框 (PS:如果选择的今日，昨日，则只填入一个日期)
...
pickerDateRange.prototype.fillDate = function (year, month, index) {
    ...
    for (var d = dateBegin; d.getTime() <= dateEnd.getTime(); d.setDate(d.getDate() + 1)) {
        if (d.getTime() < firstDayOfMonth.getTime()) { // 当前月之前的日期
            ...
            tdClass = this.mOpts.theme + '_' + this.mOpts.disableGray;
            deviation = '1';
        } else if ((this.mOpts.stopTodayBefore == true && (today.getTime() - d.getTime()) / 3600000 > 24) || (this.mOpts.stopToday == true && d.getTime() > today.getTime()) || d.getTime() < __method.str2date(__method.mOpts.minValidDate).getTime() || ('' !== __method.mOpts.maxValidDate && d.getTime() > __method.str2date(__method.mOpts.maxValidDate).getTime())) { // 当前时间之后的日期，或者开启统计之前的日期
            tdClass = this.mOpts.theme + '_' + this.mOpts.disableGray;
            deviation = '2';
        } else { // 当前月日期
            deviation = '0';

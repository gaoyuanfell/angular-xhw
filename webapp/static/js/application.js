...
app.factory("InterceptorHttp", [function () {
    return {
        ...
        response: function (response) {
            //默认为 ng-include 的请求
            if(response.data && typeof response.data == 'string' && response.config.method == 'GET' && response.config.url.lastIndexOf('.html') != -1){
                return response
            }
            //登陆拦截
...
app.directive('ycPage',['$compile',function($compile){
    return {
        ...
        template:'<div class="yc-page"><div class="item"> <ul ng-click="search($event)"><li class="top" ng-class="{\'disabled\':page.page == 1}">上一页</li><li ng-repeat="p in pageList track by $index" ng-class="{\'select\':p.n == page.page}">{{p.n}}</li><li class="bottom" ng-class="{\'disabled\':page.page == page.total_page}">下一页</li></ul></div><div class="item"><span class="go" ng-click="search($event)">跳转到</span><span><input ng-model="go" type="text"></span></div></div>',
        link:function(scope, element, attrs){
            var total_page = 1;
            var page = 1;
            scope.$session = true;
            ...
            scope.search = function(e){
                ...
                switch (n) {
                    ...
                    case '跳转到':
                        ...
                        var g = /^\d/ig;
                        scope.go = String(scope.go).replace(/[^0-9]/ig,'') || 1
                        if(scope.go <= 0){
                            scope.go = 1;
                        }
                        if(scope.go >= tp){
...
/*input 过滤*/
app.directive('regInputFtr', [function() {
    return {
        require: '?ngModel',
        link: function(scope, element, iAttrs, modelCtrl) {
            element.on('input',function () {
                var attr = iAttrs.regInputFtr;
                if(attr){
                    var reg = new RegExp(attr,'g');
                    modelCtrl.$parsers.push(function(input) {
                        var _input = String(input).replace(reg,'');
                        if (_input != input) {
                            modelCtrl.$setViewValue(_input);
                            modelCtrl.$render();
                        }
                        return _input;
                    });
...
app.factory("ResAdvertisingFty", ["$http", function ($http) {
    var invalidAdApi = baseUrl + '/invalidAD/one.htm';//作废广告位；
    var invalidAdListApi = baseUrl + '/invalidAD/list.htm';//作废广告位；
    //打底广告--更新
    ...
    //打底广告--撤销：
    var cancelDefaultApi = baseUrl + '/ADSpace/cancelDefault.htm';
    return {
        cancelDefault: function (query) {
            return $http.post(cancelDefaultApi, query, configJson);
        },
        ...
        remove: function (query) {
            return $http.post(removeApi, query, configJson);
        },
        reStart: function (query) {
            return $http.post(reStartApi, query, configForm);
        },
        invalidAD: function (query) {
            return $http.post(invalidAdApi, query, configForm);
        },
        invalidAdList:function (query) {
            return $http.post(invalidAdListApi, query, configForm);
        }
    };
}]);
...
app.factory('ConfigFty',['$http',function($http){
    var detailApi = baseUrl + '/config/detail.htm';
    var upBookTimeApi = baseUrl + '/config/upBookTime.htm';
    return {
        detail: function (query) {
            return $http.post(detailApi,query,configForm);
        },
        upBookTime: function (query) {
            return $http.post(upBookTimeApi,query,configForm);
        }
    }
}])

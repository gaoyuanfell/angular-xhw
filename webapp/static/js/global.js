    ...
    $(document.body).keyup(function (event) {
        ...
        if (key == 13) {
            ...
            if (input.length > 0) {
                ...
                if(p){
                    var i = p.querySelector('i.yc-icon');
                    i && i.click();
                }
            }
        }
...
/**
 *  图片SWF 预览
 * @param data {width height src size fileType style}
 * @returns {string}
 */
function showMaterials(data, data2) {
    var html = '';
    ...
    function Photo(data) {
        ...
        switch (+type) {
            case 0:
                photo = a + "<img style='display: block;' src='" + data.src + "' width='" + wh[0] + "' height='" + wh[1] + "'>";
                break;
            case 1:

                break;
            case 2:
                photo = a + "<video autoplay='autoplay' controls='controls' src='" + data.src + "' width='" + wh[0] + "' height='" + wh[1] + "' controls='controls'>您的浏览器不支持预览视频。</video>";
                break;
        }
        ...
        return "<div style='display: inline-block;position: relative;" + style + "'>" + photo + adMark + "</div>";
    }

    data && (html += Photo(data));
    data2 && (html += Photo(data2), width = width * 2);

    return "<div style='margin: 0 auto;position: relative;width: " + width + "px;height: " + height + "px;'>" + html + "</div>";
}

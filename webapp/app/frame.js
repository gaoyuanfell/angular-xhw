window.addEventListener('load', function () {
    ...
    menu && menu.addEventListener('click', function (e) {
        ...
        var tar = e.target;
        var ele = tar.nodeName == 'P' ? tar : tar.parentNode;
        if (ele.nodeName == 'P') {
...
app.controller('menuCtrl', ['$scope', 'SysRuleUserFty', 'SysLoginUserFty', 'SysNoticeFty', 'SysUserFty',
    function ($scope, SysRuleUserFty, SysLoginUserFty, SysNoticeFty, SysUserFty) {
        ...
        setInterval(function () {
            SysLoginUserFty.loginUserInfo().success(function(res){
                if(res){
        ...
        $scope.$on('ruleList-finish', function () {
            $R.start('Index');
            //不是谷歌浏览器 采用自定义滚动条
            if(!/Chrome/.exec(navigator.userAgent) || /Edge/.exec(navigator.userAgent)){
        ...
        $scope.userInfo = function () {
            ...
            SysUserFty.userInfo({id: $scope.user.id}).success(function (data) {
                if (data) {
                    $scope.user = data;
                }
            })
        ...
        $scope.loginOut = function () {
            SysLoginUserFty.loginOut().success(function (data) {
                if (data && data.code == 200) {
                    window.sessionStorage.clear();
        ...
        $scope.getInfoNotice = function (item, $event) {
            SysNoticeFty.noticeRead({id: item.id}).success(function () {
                SysNoticeFty.viewNotice({readState: 0}).success(function (res) {
                    if (res && res.code == 200) {
        ...
        $scope.passwordEdit = function () {
            ...
            $scope.validation = function () {
                SysUserFty.validPwd({id: $scope.user.id, logPwd: md5($scope.user.oldPassword)}).success(function (res) {
                    if (res && res.code == 200) {
        ...
        SysRuleUserFty.getUserRights().success(function (res) {
            if (res && res.code == 200) {
                var items = res.items;
